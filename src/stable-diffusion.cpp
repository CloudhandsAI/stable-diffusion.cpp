...
public:
    ...
    // hold all three at once. Opt-in (CLOUDHANDS_SEQUENTIAL_LOAD), single-DiT
    // only, and backend-agnostic (Vulkan/CPU) — unlike the CUDA-only path in #1470.
    bool                  seq_load_env      = false;  // requested via env
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
...

        {
            ...
            seq_load = seq_load_env && diffusion_model && !high_noise_diffusion_model;
            if (seq_load) {
...
void sd_img_gen_params_init(sd_img_gen_params_t* sd_img_gen_params) {
    *sd_img_gen_params = {};
    ...
    sd_img_gen_params->vae_tiling_params = {false, false, 0, 0, 0.5f, 0.0f, 0.0f, nullptr, true};  // auto-fallback default on (#1621)
